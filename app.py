...
if is_admin:
    ...
    if pwd == ADMIN_PASS:
        ...
        if not df_all.empty:
            ...
            st.markdown(
                f'<div class="header-box"><p class="total-label">{target_month} 全員合計</p><p class="total-a">{int(total_admin):,} 円</p></div>',
            ...
            for idx, row in user_summary.iterrows():
                ...
                if show_detail:
                    ...
                    rows_html = "".join([
                        f"<tr><td>{r['日付'].strftime('%m-%d')}</td><td>{r['支払先']}</td><td>{r['品名・名目']}</td><td>{r['備考']}</td><td>{int(r['金額']):,}円</td></tr>"
                        for _, r in u_detail.iterrows()
                    ])
                    st.markdown(
                        f'<table class="table-style"><thead><tr><th class="col-date">日付</th><th class="col-payee">支払先</th><th class="col-item">品名</th><th class="col-memo">備考</th><th class="col-amount">金額</th></tr></thead><tbody>{rows_html}</tbody></table>',
                        unsafe_allow_html=True
                    )
    ...
    if selected_user != "選択してください":
        ...
        if user_pwd == USER_PASS:
            ...
            st.markdown(
                f'<div class="header-box"><p class="total-label">{selected_user} さんの合計</p><p class="total-a">{int(total_val):,} 円</p></div>',
                unsafe_allow_html=True
            )

            # 新規入力フォーム（チェック位置を画像のように右側へ）
            ...
            with c1:
                ...
                # 支払先：ラベル + チェックを同じ行（右側）
                p_lbl, p_chk = st.columns([3, 1], vertical_alignment="center")
                with p_lbl:
                    st.markdown("**支払先**")
                with p_chk:
                    use_payee_history = st.checkbox("履歴選択", key="payee_history")

                # 入力欄（ラベルは消す）
                payee = st.text_input("支払先", placeholder="例：〇〇商事", label_visibility="collapsed")

            with c2:
                # 品名・名目：ラベル + チェックを同じ行（右側）
                i_lbl, i_chk = st.columns([3, 1], vertical_alignment="center")
                with i_lbl:
                    st.markdown("**品名・名目**")
                with i_chk:
                    use_item_history = st.checkbox("履歴選択", key="item_history")

                # 入力欄（ラベルは消す）
                item_name = st.text_input("品名・名目", placeholder="例：交通費", label_visibility="collapsed")
