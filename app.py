...
<style>
    ...
    * {{ font-family: 'Mochiy Pop One', sans-serif !important; }}
    ...
    /* Streamlitのチェックボックスの余計な余白を完全に削る */
...
if is_admin:
    ...
    if pwd == ADMIN_PASS:
        ...
        if not df_all.empty:
            ...
            for idx, row in user_summary.iterrows():
                ...
                with c_switch: show_detail = st.toggle("明細", key=f"det_{idx}")
            ...
            st.download_button("CSVダウンロード", admin_df.drop(columns=['年月']).to_csv(index=False).encode('utf_8_sig'), f"sum_{target_month}.csv")
else:
    # --- 個人申請モード ---
    ...
    if selected_user != "選択してください":
        ...
        if user_pwd == USER_PASS:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.markdown('<div class="align-row"><span class="align-label">日付</span></div>', unsafe_allow_html=True)
                input_date = st.date_input("日付", date.today(), label_visibility="collapsed")
                
                # 支払先ラベル + チェックボックス（HTMLで一体化）
                st.markdown('<div class="align-row"><span class="align-label">支払先</span>', unsafe_allow_html=True)
            ...
            with c2:
                # 品名・名目ラベル + チェックボックス（HTMLで一体化）
                st.markdown('<div class="align-row"><span class="align-label">品名・名目</span>', unsafe_allow_html=True)
                use_item_h = st.checkbox("履歴選択", key="use_i_h")
                st.markdown('</div>', unsafe_allow_html=True)
                if use_item_h:
                    item_name = st.selectbox("品名(履歴)", [""] + get_h("品名・名目"), label_visibility="collapsed")
                else:
                    item_name = st.text_input("品名入力", placeholder="交通費", label_visibility="collapsed")
                
                st.markdown('<div class="align-row"><span class="align-label">金額 (円)</span></div>', unsafe_allow_html=True)
                amount_str = st.text_input("金額入力", placeholder="数字を入力", label_visibility="collapsed")
            
            st.markdown('<div class="align-row"><span class="align-label">備考</span></div>', unsafe_allow_html=True)
