    ...
    if selected_user != "選択してください":
        ...
        if user_pwd == USER_PASS:
            ...
            with c1:
                ...
                input_date = st.date_input("日付ラベル隠し", date.today(), label_visibility="collapsed")
                
                # 支払先 + 履歴選択
                st.markdown('<div class="label-row"><span class="custom-label">支払先</span></div>', unsafe_allow_html=True)
                # ラベル行にチェックボックスを重ねるための擬似的な配置
                pay_hist = st.checkbox("履歴選択", key="pay_hist", help="過去の支払先から選択")
                
                if pay_hist and not df_all.empty:
                    payee = st.selectbox("支払先", options=sorted(df_all["支払先"].unique()), label_visibility="collapsed")
                else:
            ...
            with c2:
                ...
                item_hist = st.checkbox("履歴選択", key="item_hist")
                
                if item_hist and not df_all.empty:
                    item_name = st.selectbox("品名", options=sorted(df_all["品名・名目"].unique()), label_visibility="collapsed")
                else:
                ...
                amount_str = st.text_input("金額 (円)", placeholder="数字を入力", label_visibility="collapsed")

            st.markdown('<div class="label-row"><span class="custom-label">備考</span></div>', unsafe_allow_html=True)
            memo = st.text_area("備考", placeholder="補足があれば入力", height=70, label_visibility="collapsed")
...
components.html("""
    ...
    setInterval(() => {
        ...
        inputs.forEach(input => {
            if (input.placeholder && input.placeholder.includes('数字')) {
                input.type = 'number';
